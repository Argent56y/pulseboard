...
export function InboxClient({ posts, themes = [], themeLinks = [], duplicateLinks = [], imports = [], workspaceSlug = "demo", workspaceId, boardId, initialSelected, readOnly = false }: InboxClientProps) {
  ...
  const selectedThemeLinks = themeLinks.filter((link) => link.feedbackId === selectedId && link.state !== "rejected");
  const selectedDuplicates = duplicateLinks.filter((link) => link.feedbackId === selectedId && link.state !== "rejected");
